...
var LogHtml = '<script>function SendLog(){var sLog=document.getElementById("loggerTable").innerHTML;if(!confirm("log will send to support@mobitnt.com")){return};var replyMail=document.getElementById("LogRepyMail").value;if(replyMail==null||replyMail.indexOf("@")<=0){alert("invalid mail address");return};var sUrl="http://www.mobitnt.com/detect-phone/GetLog.php";var sPostData="MBLOG="+encodeURIComponent(sLog)+"&REPLYMAIL="+encodeURIComponent(replyMail);if(xmlHttp==null){createXMLHttpRequest();if(xmlHttp==null){return}};xmlHttp.onreadystatechange=xmlHttpChange;xmlHttp.open("POST",sUrl,true);xmlHttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded");xmlHttp.send(sPostData)};var xmlHttp=null;function createXMLHttpRequest(){if(xmlHttp!=null){return};if(window.XMLHttpRequest){xmlHttp=new XMLHttpRequest()}else if(window.ActiveXObject){try{xmlHttp=new ActiveXObject("Msxml2.XMLHTTP")}catch(e){try{xmlHttp=new ActiveXObject("Microsoft.XMLHTTP")}catch(e){}}}};function xmlHttpChange(){if(xmlHttp.readyState==4){alert("log data is sent to support@mobitnt.com")}};</script><p><input id="LogRepyMail"type="text"/>Enter Your Email here so we can reach you later</p><a href="#"onclick="SendLog();">send log</a><p></p><hr/>';
...
if(!obj._window.document.getElementById('loggerTable')){obj._window.document.writeln(LogHtml + "<table width='100%' id='loggerTable'><tr><th align='left'>Time</th><th width='100%' colspan='2' align='left'>Message</th></tr></table>");obj._window.document.close();}}
